...
- `iso_file` (string) - Path to the ISO file to boot from. This is expressed as a
  proxmox datastore path in the format of `<datastore>:iso/<filename>`, for example
...
- `username` (string) - Username when authenticating to Proxmox, including
  ...
  Can also be set via the `PROXMOX_USERNAME` environment variable. Note: The default
  root user account is `root@pam`.
...
source "proxmox_iso" "documentation" {
  ...
  proxmox_url = "https://node01.demo.local:8006/api2/json"
  #token = ""
  username = "root@pam"
}
```
...
- `vga` (object) - The graphics adapter to use. Example:
  ...
  + `memory` (int) - How much memory to assign in MB.
